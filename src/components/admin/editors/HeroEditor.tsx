      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-900/30 border border-slate-800/50 p-6 rounded-2xl space-y-4">
            ...
            <div className="space-y-2">
              <label className="text-xs text-gray-400 font-medium">Upload Hero Image from Admin Page</label>
              <div className="flex items-center gap-3">
                <label className="flex-1 flex flex-col items-center justify-center p-4 border-2 border-dashed border-slate-700 hover:border-accent rounded-xl cursor-pointer bg-black/40 transition-colors">
                  <ImageIcon className="w-6 h-6 text-accent mb-1" />
                  <span className="text-xs text-gray-300 font-medium">Click to upload image file or drag & drop</span>
                  <span className="text-[10px] text-gray-500 mt-0.5">PNG, JPG, WEBP, GIF up to 5MB</span>
                  <input
                    type="file"
                    accept="image/*"
                    className="hidden"
              ...
              <input
                ...
                className="w-full bg-navy border border-slate-800 p-3 rounded-lg text-xs text-gray-300 font-mono mt-2"
                placeholder="Or paste image URL (https://...)"
              />
              <p className="text-[10px] text-gray-500">Provide an image or upload a picture to enable a magnificent side-by-side split viewport.</p>
              ...
              {imageUrl ? (
                <div className="relative h-44 w-full rounded-xl overflow-hidden border border-slate-800 mt-2 group">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex items-end p-3 justify-between">
                    ...
                    <button
                      onClick={() => setImageUrl('')}
                      className="bg-rose-500/80 hover:bg-rose-500 text-white p-1 rounded text-xs cursor-pointer"
                      title="Remove image"
                    >
                      Remove
                    </button>
                ...
                <div className="h-28 w-full rounded-xl bg-black/20 border border-dashed border-slate-800 flex flex-col items-center justify-center text-gray-500 text-xs">
                  ...
                  No image specified. Hero will run on centered, full-width mode.
                </div>
              )}
            </div>
          </div>
